...
def _parse_amount_input(value: str) -> Optional[float]:
    raw = (value or "").strip()
    if not raw:
        return None
    normalized = raw.replace(" ", "")
...
async def login_post(request: Request, username: str = Form(""), password: str = Form("")):
    if not _is_auth_enabled():
        return RedirectResponse(url="/", status_code=302)
    valid_password = AUTH_USERS.get((username or "").strip())
    ...
    token = _sign_auth_payload(username.strip(), expires_at)
    ...
    try:
        if is_auto:
            ...
            while remaining and remaining > 0:
                ...
                result = perform_withdraw(part)
                sent_total += part
                sent_count += 1
                remaining = round(remaining - part, 8)
                maybe_tid = extract_tid(result)
                if maybe_tid:
                    last_tid = maybe_tid
        else:
            result = perform_withdraw(single_amount)
